...
def learn (data, vocab, K, eta=0, gamma=0, iters=1):
    ...
    for iter in range(iters):
        ...
        for i in range(N):
            if ((i+1) % INTERVAL == 0):
                eprintf ('\rGibbs sampling %d/%d documents..' % (i+1,N))
            n = seq[i]
            k = zz[n]
            pk = k
            doc = data[n]
        ...
        eprintf ('\r\x1b[Kiter[%2d] : PPL = %.2f (change=%.1f%%)\n' \
                 % (iter+1, ppl, (change/N) * 100))

    print ('Number of documents on topics:')
    nki = [int(nk[k]) for k in range(K)]
    print (nki)
