...
def uniqueness (topicwords):
    ...
    V = setsize (topicwords)
    H = entropy (tf)
    return H / log(V)

def entropy (tf):
    s = 0
    Z = np.sum (list (tf.values()))
    for word,count in tf.items():
        tf[word] = count / Z
    for word,p in tf.items():
        s -= p * log(p)
    return s

def setsize (topicwords):
    return sum ([len(words) for words in topicwords])

def uniqueness0 (topicwords):
    K = len(topicwords)
    tf = topicfreq (topicwords)
...
def usage ():
    ...
    print ('$Id: uniqueness++.py,v 1.1 2023/06/15 01:19:28 daichi Exp $')
    sys.exit (0)
