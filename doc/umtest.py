...
#    $Id: umtest.py,v 1.1 2023/02/28 23:29:50 daichi Exp $
#
...
def learn (data, K, eta=0, gamma=0, epsilon=1e-3, maxiter=1):
    #
    # prepare
    #
    ...
    for iter in range(maxiter):
        ...
        if iter > 0:
            ...
            p[0] = [0.4,0.6]; p[1] = [0.6,0.4]; p[2] = [0.4,0.6]
        # M step
        for k in range(K):
    ...
    return { 'K': K, 'lambda': exp(lamb), 'beta': exp(beta), 'nk': np.sum (p,0) }
...
def usage ():
    ...
    print ('$Id: umtest.py,v 1.1 2023/02/28 23:29:50 daichi Exp $')
    sys.exit (0)
...
def main ():
    ...
    model = learn (data, K, eta, gamma, epsilon, maxiter)
    save (model, output) 
