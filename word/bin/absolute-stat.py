...
#    $Id: absolute.py,v 1.6 2022/01/22 14:08:50 daichi Exp $
#
import sys
import putil
import numpy as np
...
def plot_data (data):
    ...
    for c,e in data.items():
        if c <= cmax:
            stat[c] = e
    xx = np.arange (1,cmax+1)
    # plot
    figure (figsize=(6,4))
    putil.fontsize (20)
    plot (xx, stat[1:], color='k')
...
def stat (first, second):
    for (word,cnt) in sorted (first.items(), key=lambda x: x[1], reverse=True):
        if (word in second):
            cnt2 = second[word]
        else:
            cnt2 = 0
        print ('%s\t%d -> %d' % (word, cnt, cnt2))
...
def usage ():
    print ('usage: % absolute-stat.py n text')
    print ('$Id: absolute.py,v 1.6 2022/01/22 14:08:50 daichi Exp $')
    sys.exit (0)
